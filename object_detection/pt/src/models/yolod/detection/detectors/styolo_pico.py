...
from object_detection.pt.src.models.yolod.detection.backbones.stresnet_pico import STResNetPico
...
ckpt_path  = "/public-shared/quark/quarknet/checkpoints/stresnet_pico.pth.tar"
...
class STYOLOPico():
    ...
    def __init__(self, cfg):
        ...
        self.backbone = STResNetPico(out_features=out_features)
        if cfg.model.pretrained: 
            ckpt = torch.load(Path(CHECKPOINT_STORAGE_URL, model_checkpoints["st_resnetpico_actrelu_pt_datasetimagenet_res224"]), weights_only=False, map_location=torch.device(cfg.device))
