...
from object_detection.pt.src.models.yolod.detection.backbones.stresnet_nano import STResNetNano
...
class STYOLONano():
    ...
    def __init__(self, cfg):
        ...
        self.backbone = STResNetNano(out_features=out_features)        
        if cfg.model.pretrained : 
            ckpt = torch.load(Path(CHECKPOINT_STORAGE_URL, model_checkpoints["st_resnetnano_actrelu_pt_datasetimagenet_res224"]), weights_only=False, map_location=torch.device(cfg.device))
    ...
    def init_yolo(self, M):
        # # yolod default, better for training from scratch, more stability
        # for m in M.modules():
        #     if isinstance(m, nn.BatchNorm2d):
        #         m.eps = 1e-3
        #         m.momentum = 0.03
        
        # # pytorch default, better for finetuning 
        for m in M.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eps = 1e-5
