...
from object_detection.pt.src.utils.yolod import bboxes_iou, cxcywh2xyxy, meshgrid, visualize_assign

from ...losses.losses import IOUloss
from ...blocks.network_blocks import BaseConv, DWConv


class YOLODHead(nn.Module):
    def __init__(
        self,
        cfg, 
        num_classes,
        ...
        self.img_w = self.cfg.model.input_shape[1]
        self.img_h = self.cfg.model.input_shape[2]
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                if not self.decode_in_inference:    
                    reg = reg_output 
                    cls = torch.cat(
                        [obj_output.sigmoid(), cls_output.sigmoid()], 1
                    )
        ...
        if self.training:
            ...
            if not self.decode_in_inference:
                reg_outputs = torch.cat(
                    [x.flatten(start_dim=2) for x in reg_outputs], dim=2
                ).permute(0, 2, 1)
                
                cls_outputs = torch.cat(
                    [x.flatten(start_dim=2) for x in cls_outputs], dim=2
                ).permute(0, 2, 1)
            ...
            if self.decode_in_inference:
                return self.decode_outputs(outputs, self.img_w, self.img_h, dtype=xin[0].type())
            else:
                # return output    
                outputs = torch.cat([reg_outputs, cls_outputs], dim=2)
                return outputs
    ...
    # def decode_outputs(self, outputs, img_w, img_h, dtype):
    #     grids = []
    #     strides = []
    ...
    #     boxes = outputs[...,0:4]
    #     prob_conf = outputs[...,4:]
    #     boxes[..., 0:2] =  (boxes[..., 0:2] + grids) * strides
    #     boxes[..., 2:4] = torch.exp((boxes[..., 2:4])) * strides
