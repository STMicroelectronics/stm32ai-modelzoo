...
from torch.nn import BatchNorm2d, Conv2d, ModuleList, ReLU, Sequential

from object_detection.pt.src.models.ssd.backbones.stresnet_micro import \
    STResNetMicro
from object_detection.pt.src.models.ssd.detectors.config import \
    vgg_ssd_config as config
...
class STResNetSSD(nn.Module):
    def __init__(self, backbone='resnet18'):
        super().__init__()
        backbone = STResNetMicro()
        
        ckpt_path  = ""        
        ckpt = torch.load(ckpt_path)
        print (f'STResNet pretrained weights loaded.')
...
def create_stresnet_ssd(num_classes, is_test=False, backbone="resnet18"):
    net = STResNetSSD()
...
def create_stresnetssd_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=None):
