...
from object_detection.pt.src.models.ssd.backbones.squeezenet import squeezenet1_1

from .ssd import SSD
from .predictor import Predictor
from .config.squeezenet_ssd_config import SQUEEZENET_CONFIG
...
def create_squeezenet_ssd_lite(num_classes, is_test=False):
    ...
    return SSD(num_classes, base_net, source_layer_indexes,
               extras, classification_headers, regression_headers, is_test=is_test, config=SQUEEZENET_CONFIG())


def create_squeezenet_ssd_lite_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=torch.device('cpu')):
    predictor = Predictor(net, SQUEEZENET_CONFIG().image_size, SQUEEZENET_CONFIG().image_mean,
                          SQUEEZENET_CONFIG().image_std,
                          nms_method=nms_method,
                          iou_threshold=SQUEEZENET_CONFIG().iou_threshold,
