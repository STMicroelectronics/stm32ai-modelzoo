...
void Display_NetworkOutput(AppConfig_TypeDef *App_Config_Ptr)
{
  ...
  pOutBuff = App_Config_Ptr->output.pOutBuff;
...
static void Display_DrawBBox(float x_center, float y_center, float width, float height, float confidence, const char* class_name, uint32_t color)
{
uint32_t res_display_width = CAM_RES_WIDTH*2; 
uint32_t res_display_height = CAM_RES_HEIGHT*2;

#if ASPECT_RATIO_MODE == KEEP_ASPECT_RATIO_PADDING
  res_display_width = CAM_RES_WIDTH*2; // 640
  res_display_height = CAM_RES_HEIGHT*2; // 480
#endif
