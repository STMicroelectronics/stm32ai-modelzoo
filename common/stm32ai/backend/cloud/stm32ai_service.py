...
from stm32ai.errors import ModelNotFoundError, WrongTypeError
from stm32ai.types import CliParameterCompression, CliParameterType
from stm32ai.types import CliParameterVerbosity, CliParameters
...
class Stm32AiService:
    ...
    def _make_trigger_request(self, route: str, options: CliParameters):
        ...
        model_is_on_cloud = False
        model_is_on_local = False

        if options.useCloud:
            cloud_models = self._get_cloud_models()
            if model_file_path in cloud_models:
                model_is_on_cloud = True
            else:
                if not os.path.exists(model_file_path):
                    raise ModelNotFoundError(f"model: {model_file_path} not \
                        found locally or on cloud")
                else:
                    model_is_on_local = True
        else:
            if not os.path.exists(model_file_path):
                cloud_models = self._get_cloud_models()
                if model_file_path in cloud_models:
        ...
        assert model_is_on_local or model_is_on_cloud, 'model_is_on_local or \
            model_is_on_cloud should be True'

        if model_is_on_local:
            files = {'model': open(model_file_path, 'rb')}
        ...
        if model_is_on_cloud:
            data['model'] = model_file_path
            headers = {'Authorization': 'Bearer '+self.auth_token}
            resp = requests.post(
                route,
                data={"args": json.dumps(data)},
