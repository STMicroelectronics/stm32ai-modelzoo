...
from stm32ai.errors import AnalyzeServerError, BenchmarkServerError
from stm32ai.errors import FileFormatError, GenerateServerError
from stm32ai.errors import InternalErrorThatShouldNotHappened
from stm32ai.errors import ParameterError, ValidateServerError
from stm32ai.errors import LoginFailureException
from stm32ai.types import AnalyzeResult, BenchmarkResult, BoardData
from stm32ai.types import GenerateResult, Stm32AiBackend, CliParameters
from stm32ai.types import ValidateResult, ValidateResultMetrics
...
class CloudBackend(Stm32AiBackend):
    ...
    def analyze(self, options: CliParameters) -> AnalyzeResult:
        ...
        if result is None or 'report' not in result\
            ...
            raise AnalyzeServerError(f'Missing data in server response')
    ...
    def generate(self, options: CliParameters) -> GenerateResult:
        ...
        if result is None or 'url' not in result:
            ...
            raise GenerateServerError(f'Missing data in server response')
    ...
    def validate(self, options: CliParameters) -> ValidateResult:
        ...
        if result is None or 'report' not in result \
            ...
            if result is None and 'message' in result:
                raise ValidateServerError(f"Missing data in server \
                    response: {result.get('message')}")
            raise ValidateServerError(f'Missing data in server response')

        report = result['report']
        graph = result.get('graph', {})
        validateResultMetrics = []
        for valmetrics in report['val_metrics']:
            validateResultMetrics.append(ValidateResultMetrics(
        ...
        validateResult = ValidateResult(
            ...
            validation_metrics=validateResultMetrics,
            ...
            report=report,
            graph=graph,
            estimated_library_flash_size=graph.get('memory_footprint', {}).get(
                'kernel_flash', None),
            estimated_library_ram_size=graph.get('memory_footprint', {}).get(
                'kernel_ram', None)
        )
