...
def _get_stm32ai_cli(root_path: Union[str, Path], host_os: str) -> List[Tuple[STMAiVersion, Path, Path]]:
    ...
    # Search for >9.0.0 to use stegdeai instead of stm32ai
    if re.findall(r"9.\d.\d", str(root_path)) or re.findall(r"\d\d.\d.\d", str(root_path)):
        exe_name = 'stedgeai.exe' if host_os == 'windows' else 'stedgeai'
    else:
    ...
    elif root_path.stem in ['stm32ai', 'stm32ai.exe', 'stedgeai', 'stedgeai.exe']:
        root_path = root_path.parent
