...
static void Display_Refresh(AppConfig_TypeDef *, bool DoRGB565toYUV422Conversion);
...
#if DISPLAY_INTERFACE == DISPLAY_INTERFACE_USB
  ret = BSP_DISPLAY_USB_Init(&config, LCD_ORIENTATION_LANDSCAPE, USB_DISP_ShowRawCb);
...
{
  uint32_t *lcd_buffer = (uint32_t *) App_Config_Ptr->lcd_frame_buff;
  
#if DISPLAY_INTERFACE == DISPLAY_INTERFACE_USB
  BSP_DISPLAY_USB_CameraCaptureBuffer(lcd_buffer, cam_buffer);
#else
#error "Selected display interface is not supported"
#endif
}
 ...
 * @param App_Config_Ptr pointer to application context
 */
static void Display_Refresh(AppConfig_TypeDef *App_Config_Ptr, bool DoRGB565toYUV422Conversion)
{
#if DISPLAY_INTERFACE == DISPLAY_INTERFACE_USB
  int ret;
