    ...
    if cfg.data_augmentation.RandomRotation:
        augmentation_layers.append(
            layers.RandomRotation(factor=cfg.data_augmentation.RandomRotation, interpolation='nearest',
                                  fill_mode='nearest'))
    if cfg.data_augmentation.RandomZoom:
        augmentation_layers.append(layers.RandomZoom(height_factor=cfg.data_augmentation.RandomZoom,
                                                     width_factor=cfg.data_augmentation.RandomZoom,
                                                     interpolation='nearest', fill_mode='nearest'))
    if cfg.data_augmentation.RandomContrast:
        augmentation_layers.append(layers.RandomContrast(factor=cfg.data_augmentation.RandomContrast))
    if cfg.data_augmentation.RandomBrightness:
        augmentation_layers.append(RandomBrightness(max_delta=cfg.data_augmentation.RandomBrightness))
    if cfg.data_augmentation.RandomShear:
        augmentation_layers.append(
            RandomShear(factor=cfg.data_augmentation.RandomShear, interpolation='nearest', fill_mode='nearest'))
