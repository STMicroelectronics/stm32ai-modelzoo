...
def get_callbacks(cfg):
    """ Get the callback choosen by the user in the config file. """
    ...
    if cfg.train_parameters.learning_rate_scheduler.lower() == 'ReduceLROnPlateau'.lower():
        # callbacks.append(
        #     tf.keras.callbacks.ReduceLROnPlateau(monitor='val_accuracy', factor=0.1, patience=20, mode='max',
        #                                          min_lr=0.000001))
        callbacks.append(
            tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', patience = 20, restore_best_weights=True))
        ...
        raise TypeError('{}  is not a valid Learning rate Scheduler, available options : {}'.format(
    ...
    checkpoint_filepath = os.path.join(HydraConfig.get().runtime.output_dir,
                                       cfg.general.saved_models_dir + '/' + "best_augmented_model.h5")
