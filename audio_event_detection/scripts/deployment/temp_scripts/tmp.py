...
def stm32ai_deploy(cfg, debug=False):
    ...
    user_files.extend([os.path.join(HydraConfig.get().runtime.output_dir, "C_header/user_mel_tables.h")])
    user_files.extend([os.path.join(HydraConfig.get().runtime.output_dir, "C_header/user_mel_tables.c")])
