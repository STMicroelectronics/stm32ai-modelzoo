...
def get_data_augmentation(cfg):
    ...
    for key, value in cfg.data_augmentation.items():
        if value:
            augment = True
    augmentation_layers = []
    if cfg.data_augmentation.GaussianNoise:
        augmentation_layers.append(GaussianNoise(stddev=cfg.data_augmentation.GaussianNoise))
    if cfg.data_augmentation.VolumeAugment:
        augmentation_layers.append(VolumeAugment(cfg.data_augmentation.min_scale,
                                                 cfg.data_augmentation.max_scale,
                                                 cfg.feature_extraction.to_db))
    if cfg.data_augmentation.SpecAug:
        augmentation_layers.append(SpecAugment(freq_mask_param=cfg.data_augmentation.freq_mask_param,
                                               time_mask_param=cfg.data_augmentation.time_mask_param,
                                               n_freq_mask=cfg.data_augmentation.n_freq_mask,
